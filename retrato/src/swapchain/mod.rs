...
impl Swapchain {
    ...
    pub fn submit(&mut self, command_buffers: Vec<vk::CommandBuffer>, index: usize) -> (u32, Result<bool, vk::Result>) {
        ...
        let suboptimal = unsafe { self.swapchain_funcs.queue_present(self.device.present_queue, &present_info) };
        self.current_frame = (self.current_frame + 1) % 2;
        return (image_index, suboptimal);
    }
    // pub fn compare_swap_formats(&self, other: &Self) -> bool {
    //     other.depth_format == self.depth_format
    // }
}
