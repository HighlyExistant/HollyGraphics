...
impl Device {
    ...
    pub fn unstable_from_winarabica(entry: &Entry, window: &winarabica::window::Window) -> Arc<Self> {
        let instance = instance::Instance::new(&entry, window.raw_display_handle());
        let surface = Self::create_surface_winarabica(&entry, &instance.instance, &window);
        let (physical_device, queue_index, surface_funcs) = Self::choose_device(&entry, &instance, &surface);
        let device = Self::create_device(&instance.instance, &physical_device, queue_index as u32);
        let present_queue = unsafe { device.get_device_queue(queue_index as u32, 0) };
        let command_pool = Self::create_commandpool(&device, queue_index as u32);
    ...
    fn create_surface_winarabica(entry: &Entry, instance: &ash::Instance, window: &winarabica::window::Window) -> vk::SurfaceKHR {
