...
pub struct BatchRenderer<'a, T: vertex::Vertex, I: model::Index, M: model::Model<T, I>> {
    models: Vec<&'a mut M>,
...
impl<'a, T: vertex::Vertex + std::fmt::Debug, I: model::Index + std::fmt::Debug, M: model::Model<T, I>> BatchRenderer<'a, T, I, M>  {
    pub fn push(&mut self, model: &'a mut M) {
    ...
    pub fn create(&mut self, allocator: &mut allocator::BufferAllocator) -> Batch::<T, I> {
        ...
        for i in 0..self.models.len() {
            ...
            let mut reference = &mut self.models[i];
...
impl<'a, T: vertex::Vertex, I: model::Index, M: model::Model<T, I>> Default for BatchRenderer<'a, T, I, M> {
    fn default() -> Self {
