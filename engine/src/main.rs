...
use components::{rendersystem::RenderSystem, models::{Mesh3D, FromFBX}, object::BasicObject, scene::Scene, physics::rigidbody::RigidBody, collisions::collider::{GJKCollisions, self}};
...
fn main() {
    ...
    let monke = Rc::new(Mesh3D::<GlobalDebugVertex>::from_fbx("monke.fbx")[0].clone());
    let cube = Rc::new(Mesh3D::<GlobalDebugVertex>::from_fbx("untitled.fbx")[0].clone());
    ...
    let vertices: Vec<_> = cube.vertices.clone().iter().map(|v| {
        v.pos
    }).collect();
    let collider1 = Rc::new(RefCell::new(GJKCollisions::new(vertices.clone())));
    ...
    let mut y= 0.0;
    {
        ...
        collider_system.push(0, collider1.clone());
        collider_system.push(1, collider1.clone());
            ...
            match event {
                ...
                winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
                    ...
                    unsafe { application.device.device.cmd_bind_descriptor_sets(cmd_buffer, vk::PipelineBindPoint::GRAPHICS, application.layout, 0, &[application.sets[application.renderer.swapchain.current_frame]], &[]) };
                    {
                        ...
                        if let Some(collider) = collider_system.get_collider_by_id(0) {
                    ...
                    drop(lock);
                    collider_system.render_all(application.device.clone(), &mut scene);
