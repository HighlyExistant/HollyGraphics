...
impl Default for Quadrilateral {
    fn default() -> Self {
        Self { a: FVec2::new(-1.0, 1.0), b: FVec2::new(1.0, 1.0), c: FVec2::new(-1.0, -1.0), d: FVec2::new( 1.0, -1.0) }
    }
}
#[derive(Default)]
...
pub fn seperate_axis(poly_a: &Polygon2D, poly_b: &Polygon2D) -> bool {
    let mut perpendicular_line = FVec2::new(0.0, 0.0);
    let mut dot = 0.0;
    ...
    for i in 0..poly_a.edges.len() {
        perpendicular_line = FVec2 { x: -poly_a.edges[i].y, y: poly_a.edges[i].x };
        perpendicular_stack.push(perpendicular_line);
    }
    
    for i in 0..poly_b.edges.len() {
        perpendicular_line = FVec2 { x: -poly_b.edges[i].y, y: poly_b.edges[i].x };
        perpendicular_stack.push(perpendicular_line);
    }
